...
export function evaluateEligibilityForWinners({ user, payment, quiz, attempt }) {
  // For winners, only check payment and attempt completion (not quiz state)
  if (!user) {
    return { eligible: false, reason: 'USER_NOT_FOUND' };
  }
  ...
  // For winners, attempt must be completed
  if (!attempt || !attempt.answersSaved) {
    return { eligible: false, reason: 'QUIZ_NOT_COMPLETED' };
  }
