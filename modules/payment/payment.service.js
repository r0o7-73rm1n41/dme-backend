...
export async function isUserEligible(userId, quizDate) {
  ...
  // Check user.quizEligibility
  const User = (await import("../user/user.model.js")).default;
  const user = await User.findById(userId).select('quizEligibility');
  if (user && user.quizEligibility && user.quizEligibility.isEligible && user.quizEligibility.eligibleDate === quizDate) {
    const eligible = true;
    // Cache eligibility for 1 hour
    try {
      await redisClient.setEx(cacheKey, 3600, JSON.stringify(eligible));
    } catch (error) {
  ...
  // Fallback to payment check for backward compatibility
  ...
  const eligible = Boolean(payment);

  // Cache eligibility for 1 hour (eligibility doesn't change frequently)
  try {
    await redisClient.setEx(cacheKey, 3600, JSON.stringify(eligible));
  } catch (error) {
    console.warn('Redis cache write failed:', error.message);
